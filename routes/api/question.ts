...
// Define the type for the questionFields object
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      for (const question of questions) {
        if (!question.answeredBy.includes(new mongoose.Types.ObjectId(userId))) {
...
const createTests = async (questionData: IQuestionData[], test_name: string) => {
...
router.get(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      for (const question of questions) {
        question.answeredBy.push(new mongoose.Types.ObjectId(userId));
        await question.save();
      }
...
router.get('/:checkedSubjects/:userId/:noofquestions', async (req: Request, res: Response) => {
  try {
